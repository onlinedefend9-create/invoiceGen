...
export const InvoicePreview: React.FC<Props> = ({ invoice, userProfile, id }) => {
  ...
  const templates = {
    minimalist: (
      <div className="space-y-12 text-slate-800 font-sans p-12 bg-white min-h-[1000px]">
        ...
        <div className="mt-auto pt-24 border-t border-slate-100 space-y-8">
          <div className="grid grid-cols-2 gap-12">
            ...
            <div className="space-y-2 text-right">
              ...
              <p className="text-xs text-slate-400 font-bold font-mono">{userProfile.rib}</p>
            </div>
          </div>
          <div className="pt-8 flex justify-between items-center text-[8px] font-black uppercase tracking-[0.2em] text-slate-300">
            <span>SIRET: {userProfile.siret}</span>
            <span className="bg-slate-50 px-3 py-1 rounded-full">{t('invoice.generatedVia', { defaultValue: 'Generated via InvoiceGen Pro' })}</span>
          </div>
        </div>
      </div>
    ),
    corporate: (
      <div className="bg-white min-h-[1000px] p-0 font-sans text-slate-800">
        <div className="bg-slate-800 text-white p-12 flex justify-between items-center">
          <div className="space-y-2">
            <h1 className="text-3xl font-black tracking-tighter">{userProfile.companyName}</h1>
            <p className="text-xs text-slate-400 font-bold uppercase tracking-widest">{userProfile.email} • {userProfile.phone}</p>
          </div>
          <div className="text-right">
            <h2 className="text-4xl font-black tracking-tighter mb-1">{getDocumentTitle(invoice.type)}</h2>
        ...
        <div className="p-12 space-y-12">
          <div className="grid grid-cols-2 gap-12">
            ...
            <div className="p-8 border border-slate-100 rounded-3xl space-y-4 text-right">
          ...
          <div className="flex justify-end">
            <div className="w-80 bg-slate-50 p-8 rounded-3xl space-y-4">
    ...
    modern: (
      <div className="bg-white min-h-[1000px] p-12 font-sans text-slate-900 relative overflow-hidden">
        ...
        <div className="relative z-10 space-y-16">
          <div className="flex justify-between items-start">
            ...
            <div className="text-right space-y-4">
              <h2 className="text-7xl font-black text-slate-100 tracking-tighter leading-none">
                {invoice.type === 'invoice' ? 'INV' : 
                 invoice.type === 'credit_note' ? 'CRD' : 
                 invoice.type === 'quote' ? 'DEV' : 'BON'}
              </h2>
              <div className="space-y-1">
                <p className="text-xl font-black tracking-tighter">{invoice.invoiceNumber}</p>
          ...
          <div className="grid grid-cols-2 gap-12">
            <div className="space-y-4">
              <h4 className="text-[10px] font-black uppercase tracking-widest text-slate-300">{t('invoice.billedTo', { defaultValue: 'Billed to' })}</h4>
              <div className="space-y-2">
                <p className="text-2xl font-black tracking-tight">{invoice.client.name}</p>
            ...
            <div className="text-right space-y-4">
              ...
              <p className="text-2xl font-black tracking-tight text-rose-500">{format(parseISO(invoice.dueDate), 'dd MMM yyyy')}</p>
          ...
          <div className="space-y-6">
            ...
            <div className="space-y-4">
              {invoice.items.map(item => (
                <div key={item.id} className="flex items-center justify-between p-8 bg-slate-50 rounded-[32px] group hover:bg-blue-50 transition-colors">
                  <div className="space-y-1">
                    <p className="font-black text-lg tracking-tight">{item.description}</p>
                    <p className="text-xs font-bold text-slate-400 uppercase tracking-widest">{item.quantity} x {formatCurrency(item.unitPrice)}</p>
                  </div>
                  <p className="text-2xl font-black tracking-tighter">{formatCurrency(item.quantity * item.unitPrice)}</p>
                </div>
              ))}
            </div>
          </div>
          ...
          <div className="flex justify-end pt-12">
            <div className="w-80 space-y-6">
              ...
              <div className="flex justify-between text-4xl font-black tracking-tighter pt-8 border-t-8 border-blue-600">
                <span>{t('invoice.total', { defaultValue: 'Total' })}</span>
                <span className="text-blue-600">{formatCurrency(total)}</span>
              </div>
            </div>
          ...
          <div className="mt-auto pt-24 space-y-12">
            ...
            <div className="flex justify-between items-center text-[10px] font-black uppercase tracking-widest text-slate-300">
              <p>SIRET: {userProfile.siret} • {userProfile.address}</p>
              <p className="bg-blue-600 text-white px-4 py-1.5 rounded-full">InvoiceGen Pro</p>
            </div>
          </div>
        </div>
      </div>
    ...
    credit_note: (
      <div className="bg-white min-h-[1000px] p-0 font-sans text-blue-900 border-4 border-blue-900">
        <div className="bg-white text-blue-900 p-12 flex justify-between items-center border-b-4 border-blue-900">
          <div className="space-y-2">
            <h1 className="text-3xl font-black tracking-tighter uppercase">{userProfile.companyName}</h1>
            <p className="text-xs text-blue-400 font-bold uppercase tracking-widest">{userProfile.email} • {userProfile.phone}</p>
          </div>
          <div className="text-right">
            <h2 className="text-4xl font-black tracking-tighter mb-1">{getDocumentTitle(invoice.type)}</h2>
            <p className="text-sm font-bold opacity-50">{invoice.invoiceNumber}</p>
        ...
        <div className="p-12 space-y-12">
          <div className="grid grid-cols-2 gap-12">
            ...
            <div className="p-8 border border-blue-100 rounded-3xl space-y-4 text-right">
          ...
          <div className="flex justify-end">
            <div className="w-80 bg-blue-50 p-8 rounded-3xl space-y-4">
        ...
        <div className="mt-auto p-12 border-t border-blue-100 flex justify-between items-center text-[10px] font-black uppercase tracking-widest text-blue-300">
          <p>SIRET: {userProfile.siret} • {userProfile.address}</p>
